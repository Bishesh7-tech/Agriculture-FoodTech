...
public class WBCropKnowledgeBase {
    ...
    public Map<String, Object> getKvkDetails(String districtName, Double latitude, Double longitude) {
        DistrictInfo district = getDistrict(districtName);
        if (district == null && latitude != null && longitude != null) {
            district = findNearestDistrict(latitude, longitude);
        }

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("available", district != null);
        result.put("district", district != null ? district.name() : districtName);
        result.put("name", district != null ? district.kvkName() : "Nearest KVK not matched to your location");
        result.put("phone", district != null ? district.kvkPhone() : "1800-180-1551");
        result.put("address", district != null ? district.kvkAddress() : "Contact your nearest district agriculture office or Kisan Call Centre for field support.");
        result.put("website", district != null ? "https://kvk.icar.gov.in/" : "https://kvk.icar.gov.in/");
        result.put("email", null);
        result.put("source", "KVK / ICAR official directory");
        result.put("message", district != null
                ? "Nearest KVK details are available for local follow-up."
                : "Official KVK details were not matched to your location, so use the district agriculture office or Kisan Call Centre for confirmation and treatment advice.");
        return result;
    }
